...
func TestNginxRPS(t *testing.T) {
	...
	if err := data.createPodOnNode("nginx-local-client", controlPlaneNodeName(), perftoolImage, nil, nil, nil, nil, false, nil); err != nil {
	...
	t.Run("testNginxRPS, InterNode", func(t *testing.T) { testNginxRPS(t, data, localSvrIPs, localSvc, intraNode) })
...
func testNginxRPS(t *testing.T, data *TestData, podIPs *PodIPs, svc *v1.Service, testType int) {
	runHey := func(cmd string) (rps float64) {
		stdout, _, err := data.runCommandFromPod(testNamespace, "nginx-local-client", "perftool", []string{"bash", "-c", cmd})
		if err != nil {
	...
	for i := 0; i < roundNum; i++ {
		cmd = fmt.Sprintf("ab -c 1 -n 30000  http://%s/  |grep 'Requests per second:'|awk '{print $4}'", podIPs.ipv4.String())
		acc += runHey(cmd)
	}
	...
	acc = 0
	for i := 10000; i <= 10002; i++ {
		cmd = fmt.Sprintf("ab -c 1 -n 30000  http://%s/  |grep 'Requests per second:'|awk '{print $4}'", svc.Spec.ClusterIP)
		acc += runHey(cmd)
	}
